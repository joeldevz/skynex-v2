...
assert.deepEqual(args.filter((_,i)=>i%2===0).sort(), ["--command","--installed-config-dir","--root"].sort(), "Usage: verify-installed-opencode2.mjs --command PATH --installed-config-dir DIR --root ROOT");
const command=resolve(value("--command"));
const root=await realpath(resolve(value("--root")));
const installedArgument=resolve(value("--installed-config-dir"));
const project=join(root,"project");
const configDir=join(root,"runtime-config","opencode");
const env={ PATH:"/usr/bin:/bin", HOME:join(root,"runtime-home"), XDG_CONFIG_HOME:join(root,"runtime-config"), XDG_DATA_HOME:join(root,"runtime-data"), XDG_STATE_HOME:join(root,"runtime-state"), XDG_CACHE_HOME:join(root,"runtime-cache"), TMPDIR:join(root,"runtime-tmp") };
const installedPackageRoot = resolve(installedArgument, "../../prefix/lib/node_modules/@skynex-ai/cli");
const { parse } = createRequire(join(installedPackageRoot, "package.json"))("jsonc-parser");
const evidence = { integrationVerified: false, mode: "installed-snapshot",
...
try {
  ...
  const installed = await realpath(installedArgument);
  assert(installed.startsWith(`${root}/`), "Installed fixture must remain under root");
