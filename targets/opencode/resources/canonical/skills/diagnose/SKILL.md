...
<!-- skynex:autonomous-diagnosis-secure-gateway:v1 -->
...
Write the regression test **before the fix** only when `slice.tdd=true`. When `slice.tdd` is false or absent, a postimplementation regression test is allowed.
